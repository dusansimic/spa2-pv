...
public class PostojanjePuta {
    ...
    private static boolean postojiPut(int x, int y, List<String> polja, boolean rupa, int visinaPrethodnogPolja, int preskocene){
        ...
        if (postojiPut(x+1,y, polja, lavirint[x][y] == -1, lavirint[x][y], preskocene))
            return true;

        if (postojiPut(x-1, y, polja, lavirint[x][y] == -1, lavirint[x][y], preskocene))
            return true;

        if (postojiPut(x, y+1, polja, lavirint[x][y] == -1, lavirint[x][y], preskocene))
            return true;

        if (postojiPut(x, y-1, polja, lavirint[x][y] == -1, lavirint[x][y], preskocene))
            return true;

        // Dijagonalno kretanje omogucava trazenje resenja u primeru lavb.txt
        if (postojiPut(x+1,y+1, polja, lavirint[x][y] == -1, lavirint[x][y], preskocene))
            return true;
